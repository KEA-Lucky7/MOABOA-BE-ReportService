pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = 'lucky7-cr/report-service'
    }
    stages {
        stage('Checkout') {
            steps {
                git branch: 'main', credentialsId: 'github-signin', url: 'https://github.com/KEA-Lucky7/MOABOA-BE-ReportService.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Update Helm Values YAML') {
            steps {
                script {
                    ...
                    sh """
                    sed -i 's|tag: .*|tag: ${env.BUILD_ID}|' charts/report-service/values.yaml
                    """
                    withCredentials([usernamePassword(credentialsId: 'github-signin', usernameVariable: 'GIT_USERNAME', passwordVariable: 'GIT_PASSWORD')]) {
                        ...
                        git add charts/report-service/values.yaml
                        git commit -m "[UPDATE] report service image tag ${env.BUILD_ID}"
